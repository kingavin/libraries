...
class App_Factory
{
	...
	/**
	 * Construct a mongo collection model to read mongo database
	 * @param String $modelName
	 * @return App_Mongo_Db_Collection
	 */
	static public function _m($modelName)
	{
		if(empty($modelName)) {
			throw new Exception('could not find model list class');
		}
	 ...
	 * @return App_Mongo_Db_Collection
	 */
	static public function _am($modelName)
	{
		if(empty($modelName)) {
			throw new Exception('could not find model list class');
		}
