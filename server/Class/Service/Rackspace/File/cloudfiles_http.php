...
class CF_Http
{
    ...
    protected function _header_cb($ch, $header)
    {
    ...
    protected function _read_cb($ch, $fd, $length)
    {
    ...
    protected function _write_cb($ch, $data)
    {
        $dlen = strlen($data);
    ...
    protected function _auth_hdr_cb($ch, $header)
    {
        preg_match("/^HTTP\/1\.[01] (\d{3}) (.*)/", $header, $matches);
        if (isset($matches[1])) {
            $this->response_status = $matches[1];
    ...
    protected function _make_headers($hdrs=NULL)
    {
        $new_headers = array();
        $has_stoken = False;
        $has_uagent = False;
    ...
    protected function _init($conn_type, $force_new=False)
    {
    ...
    protected function _reset_callback_vars()
    {
        $this->_text_list = array();
	$this->_return_list = NULL;
        $this->_account_container_count = 0;
    ...
    protected function _make_path($t="STORAGE",$c=NULL,$o=NULL)
    {
        $path = array();
        switch ($t) {
        case "STORAGE":
    ...
    protected function _metadata_headers(&$obj)
    {
        $hdrs = array();
        if ($obj->manifest)
    ...
    protected function _send_request($conn_type, $url_path, $hdrs=NULL, $method="GET", $force_new=False)
    {
